...
from .models import Post, Category, Profile, User, Comment, AffiliateLink, Services
...
def service_create_and_list(request):
    if request.method == 'POST':
        form = ServiceForm(request.POST, request.FILES)  # handle image uploads
        if form.is_valid():
            form.save()
    ...
    services = Services.objects.all()  # <-- use correct model name

    return render(request, 'service_form_and_list.html', {
        'form': form,
        'services': services,   # <-- match template variable
    })
